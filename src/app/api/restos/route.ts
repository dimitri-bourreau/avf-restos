...
if (!sheetID || !GID) throw new Error('Missing env variables GOOGLE_SHEET_ID || GOOGLE_SHEET_GID')
...
async function fetchSheetData(): Promise<Resto[]> {
  ...
  const nomIndex = cols.findIndex((c: { label: string }) => c.label === "Nom du resto");
  const adresseIndex = cols.findIndex((c: { label: string }) => 
    c.label === "Adresse" || c.label === "Adresse - lien site"
  );
  const statutIndex = cols.findIndex((c: { label: string }) => c.label === "Statut");
